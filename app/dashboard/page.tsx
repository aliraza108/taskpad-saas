...
export default function Dashboard() {
  ...
  const addTask = async () => {
    ...
    else if (data) setTasks([...data, ...tasks])
          ...
          <div className="flex justify-center gap-4 mb-6">
            {["all", "completed", "pending"].map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-4 py-2 rounded-xl font-semibold shadow-md ${filter === f ? "bg-blue-700" : "bg-blue-300"}`}
              >
            ...
            <ul className="space-y-4">
              {filteredTasks.map(task => (
                <li key={task.id} className="flex justify-between items-center bg-gray-50 p-4 rounded-2xl shadow-md hover:shadow-xl transition">
                  <span
                    className={`cursor-pointer font-medium text-black ${task.is_completed ? "line-through opacity-60" : ""}`}
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => toggleTask(task.id, task.is_completed)}
                      className={`px-4 py-2 rounded-lg font-semibold text-black shadow-md transition ${task.is_completed ? "bg-green-300 hover:bg-green-400" : "bg-yellow-300 hover:bg-yellow-400"}`}
                    >
